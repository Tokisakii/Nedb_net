...
if __name__ == '__main__':
  for i in range(10):
    ...
    model = nedbnet.get_seg_model(None).eval()
    model = model.cuda()
    pretrained_dict = torch.load("./5/DB-NL.pth")
    model_dict = model.state_dict()
    ...
    mask_pred = torch.sigmoid(mask_pred)
    # mask_pred = (mask_pred.squeeze(0).squeeze(0) >= 0.5).long().numpy()
    db_nl_pred = ((mask_pred.squeeze(0).squeeze(0)).cpu().detach().numpy() * 255).astype(np.uint8)
    # cv2.imshow("resnet34_cons_imp_pred", resnet34_cons_imp_pred)
    ...
    plt.imshow(resnet34_cons_emb_pred, cmap='gray')

    # 4. 双分支+噪声+边缘
    plt.subplot(1, 6, 4)
    plt.axis('off')
    plt.title("双分支+噪声+边缘", fontproperties=font)
    plt.imshow(db_pred, cmap='gray')

    # 5. 双分支+噪声+边缘+NL
    plt.subplot(1, 6, 5)
    plt.axis('off')
    plt.title("双分支+噪声\n+边缘+NL", fontproperties=font)
    plt.imshow(db_nl_pred, cmap='gray')
    # plt.imshow(resnet34_cons_emb_pred, cmap='gray')
    ...
    plt.imshow(db_nl_d_pred, cmap='gray')
